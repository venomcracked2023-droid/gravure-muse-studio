...
export const Route = createFileRoute("/featured")({
  ...
  head: () => {
    ...
    const desc = "Những album gravure mới đăng gần đây trên GravureHub, cập nhật liên tục mỗi ngày.";
  ...
  const ld = {
    ...
    mainEntity: {
      ...
      itemListElement: featured.slice(0, 30).map((a, i) => ({
        ...
        image: (a.chapter.coverId || a.chapter.pages[0] || a.comic.coverId) ? driveImageUrl(a.chapter.coverId || a.chapter.pages[0] || a.comic.coverId, 600) : undefined,
      })),
    },
  };
  return (
            ...
            {featured.map((a) => (
              ...
              >
                <div className="hover-lift relative aspect-[3/4] overflow-hidden rounded-xl border border-primary/40 bg-card">
                  <ComicCover id={a.chapter.coverId || a.chapter.pages[0] || a.comic.coverId} title={a.chapter.title} className="transition duration-500 group-hover:scale-110" />
                </div>
                <div>
                  <h2 className="line-clamp-1 text-sm font-semibold group-hover:text-primary">{a.chapter.title}</h2>
                  <p className="line-clamp-1 text-xs text-muted-foreground">{a.comic.title} · {a.comic.chapters.length} album</p>
                </div>
              </Link>
            ))}
          </div>
