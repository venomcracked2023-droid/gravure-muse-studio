...
export function CommentSection({ comicId, chapterId }: { comicId: string; chapterId?: string }) {
  ...
  async function load() {
    setLoading(true);
    let q = supabase.from("comments").select("id,user_id,content,created_at")
      .eq("comic_id", comicId).order("created_at", { ascending: false }).limit(200);
  ...
  useEffect(() => { load(); }, [comicId, chapterId]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!content) return;
    if (content.length > MAX) { toast.error(`Tối đa ${MAX} ký tự`); return; }
    setPosting(true);
    const { error } = await supabase.from("comments").insert({
      comic_id: comicId, chapter_id: chapterId ?? null, user_id: user.id, content,
    });
    setPosting(false);
  ...
  async function remove(id: string) {
    ...
    if (!user) { toast.error("Bạn cần đăng nhập lại"); return; }
    const { data, error } = await supabase.from("comments").delete().eq("id", id).select("id");
    if (error) { toast.error(error.message); return; }
    if (!data || data.length === 0) { toast.error("Không có quyền xoá"); return; }
  ...
  return (
    <section className="mt-10" aria-label="Bình luận">
      <header className="mb-4 flex items-center gap-2">
        ...
        <h2 className="text-xl font-bold">Bình luận{chapterId ? " album" : ""} ({items.length})</h2>
      </header>

      {user ? (
        <form onSubmit={submit} className="rounded-2xl border border-border bg-card/60 p-4 backdrop-blur">
          ...
          <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
            ...
            <button type="submit" disabled={posting || !text.trim()}
              ...
              <Send className="h-3.5 w-3.5" />{posting ? "Đang gửi…" : "Gửi"}
            </button>
          </div>
        </form>
      ) : (
        <div className="rounded-2xl border border-dashed border-border bg-card/30 p-4 text-sm text-muted-foreground">
          <Link to="/login" className="text-primary hover:underline">Đăng nhập</Link> để tham gia bình luận.
        </div>
      )}

      <ul className="mt-5 space-y-3">
        {loading && <li className="rounded-xl border border-border bg-card/30 p-4 text-sm text-muted-foreground">Đang tải…</li>}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {canDelete && (
                      <button onClick={() => remove(c.id)} className="text-muted-foreground hover:text-destructive" aria-label="Xoá">
                        <Trash2 className="h-4 w-4" />
                      </button>
                    )}
                  </div>
                  <p className="mt-1 whitespace-pre-wrap break-words text-sm text-foreground/90">{c.content}</p>
                </div>
              </div>
            </li>
          );
