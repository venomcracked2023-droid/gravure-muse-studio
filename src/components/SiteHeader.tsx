      ...
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
        ...
        <form onSubmit={submitSearch} role="search"
          ...
          <input type="search" value={q} onChange={(e) => setQ(e.target.value)}
            placeholder="Tìm người mẫu, album, thể loại…" aria-label="Tìm kiếm"
            className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground/70" />
          {q && (
            <button type="button" onClick={() => { setQ(""); navigate({ to: "/", search: {} }); }}
              aria-label="Xoá tìm kiếm" className="rounded-full p-0.5 text-muted-foreground hover:bg-secondary hover:text-foreground">
        ...
        <nav className="flex items-center gap-0.5 text-sm">
          ...
          {isContributor && (
            <Link to="/admin" className={navClass} activeProps={{ className: activeClass }}>
              <Settings className="h-4 w-4" /><span className="hidden sm:inline">Quản lý</span>
            </Link>
          )}
          {isAdmin && (
            <Link to="/admin-applications" className={navClass} activeProps={{ className: activeClass }}>
              <ShieldCheck className="h-4 w-4" /><span className="hidden sm:inline">Duyệt CTV</span>
            </Link>
          )}
          {user && !isContributor && (
            <Link to="/apply" className={navClass} activeProps={{ className: activeClass }}>
              <UserPlus className="h-4 w-4" /><span className="hidden sm:inline">Ứng tuyển</span>
            </Link>
          )}
          {user ? (
            <div className="ml-2 flex items-center gap-2">
              ...
              <button onClick={() => signOut()} aria-label="Đăng xuất"
