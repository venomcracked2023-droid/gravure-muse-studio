...
export function RatingWidget({ comicId }: { comicId: string }) {
  ...
  async function load() {
    const { data } = await supabase.from("ratings").select("user_id,score").eq("comic_id", comicId);
  ...
  return (
    <div className="mt-5 inline-flex items-center gap-3 rounded-full border border-border bg-card/40 px-3 py-1.5 backdrop-blur">
      <div className="flex items-center" onMouseLeave={() => setHover(0)} role="radiogroup" aria-label="Đánh giá">
        {[1, 2, 3, 4, 5].map((n) => {
          const filled = (hover || myScore) >= n;
          return (
            <button key={n} type="button" disabled={saving}
              onMouseEnter={() => setHover(n)} onClick={() => setScore(n)}
              aria-label={`${n} sao`} aria-checked={myScore === n} role="radio"
              className="p-0.5 transition disabled:opacity-50">
              <Star className={"h-5 w-5 transition " + (filled ? "fill-primary text-primary" : "text-muted-foreground")} />
            </button>
          );
        })}
      </div>
      <div className="text-xs text-muted-foreground tabular-nums">
        {rows.length > 0 ? (<><span className="font-semibold text-foreground">{avg.toFixed(1)}</span> / 5 · {rows.length} đánh giá</>) : "Chưa có đánh giá"}
      </div>
    </div>
  );
}