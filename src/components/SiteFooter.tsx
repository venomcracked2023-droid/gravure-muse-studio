    ...
    <footer className="relative mt-20 border-t border-border bg-card/40" role="contentinfo">
      ...
      <div className="mx-auto max-w-6xl px-4 py-12">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            <p className="mt-3 max-w-sm text-sm text-muted-foreground">
              Ngắm bộ ảnh gravure cuộn dọc, mượt như lụa.
            </p>
          </div>

          <nav aria-label="Khám phá">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-foreground/80">Khám phá</h3>
            <ul className="mt-3 space-y-2 text-sm">
              <li><Link to="/" className="text-muted-foreground hover:text-primary">Trang chủ</Link></li>
              <li><Link to="/featured" className="text-muted-foreground hover:text-primary">Nổi bật</Link></li>
              <li><Link to="/latest" className="text-muted-foreground hover:text-primary">Mới cập nhật</Link></li>
            </ul>
          </nav>

          {toc.length > 0 && (
            <nav aria-label="Người mẫu mới">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-foreground/80">Người mẫu mới</h3>
              <ul className="mt-3 space-y-2 text-sm">
                {toc.map((c) => (
                  <li key={c.id}>
          ...
          {genres.length > 0 && (
            <nav aria-label="Thể loại">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-foreground/80">Thể loại</h3>
              <ul className="mt-3 space-y-2 text-sm">
                {genres.map((g) => (
        ...
        <div className="mt-10 flex flex-col items-start justify-between gap-3 border-t border-border/60 pt-6 text-xs text-muted-foreground sm:flex-row sm:items-center">
          <p>© <time dateTime={String(year)}>{year}</time> {SITE_NAME}. Mọi quyền được bảo lưu.</p>
